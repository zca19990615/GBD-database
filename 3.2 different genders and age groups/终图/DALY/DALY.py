...
filtered_activity_data = data[(data['rei'] == 'Low physical activity') & (data['year'] == 2021) & (data['sex'] == 'Female')]

# 创建透视表，索引为年龄组，列为位置，值为DALYs率
pivot_activity_data = filtered_activity_data.pivot_table(index='age', columns='location', values='val')

# 设置风格为自然风格
sns.set(style="whitegrid", palette="muted")

# 修改配色
colors = {'Global': '#1f77b4', 'China': '#d62728'}  # 蓝色和红色
...
plt.xlabel('Female Age Group', fontsize=30)
plt.ylabel('DALY Rate (per 100,000)', fontsize=30)
plt.xticks(rotation=45, fontsize=28)
plt.yticks(fontsize=28)
plt.legend(title='Location', fontsize=28, title_fontsize=30, loc='upper left')
plt.grid(False)
plt.tight_layout()
